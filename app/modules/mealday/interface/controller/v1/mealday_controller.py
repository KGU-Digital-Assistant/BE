...
mealday_router = APIRouter(prefix="/api/v1/meal-days", tags=["MealDay"])
...
@mealday_router.post("/{track_id}", response_model=APIResponse)
@inject
...
dish_router = APIRouter(prefix="/api/v1/dishes", tags=["Dish"])
...
@dish_router.patch("/{dish_id}/image-url", response_model=DishImageUrl)
@inject
